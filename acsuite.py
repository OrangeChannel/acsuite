...
class AC:
    ...
    def eztrim(self, clip: vs.VideoNode, trims: Union[List[Tuple[int, int]],
        ...
        :param clip: needed to determine framerate for audio timecodes
                     and `num_frames` for negative indexing
            :bit depth: ANY
            :color family: ANY
            :float precision: ANY
            :sample type: ANY
        ...
        :param outfile: either a filename `'out.wav'`
                        or a full path `'/path/to/out.wav'`

        :param debug: used for testing, leave blank (Default = `False`)

        OUTPUTS: a cut/spliced audio file in either the script's
                 directoy or the path specified with 'outfile'
        """
        self.__init__()

        if not isinstance(trims, (list, tuple)):
            raise TypeError('eztrim: trims must be a list of 2-tuples '
                            '(or just one 2-tuple)')
        ...
        for trim in trims:
            if type(trim) == int:
                # if first element is an int, assume it's a single tuple
                single = True
                if len(trims) != 2:
                    raise ValueError('eztrim: the trim must have 2 '
                                     'elements')
                break

            else:  # makes sure to error check only for multiple tuples
                if not isinstance(trim, (list, tuple)):
                    raise TypeError('eztrim: the trim {} is not a tuple'
                                    .format(trim))
                if len(trim) != 2:
                    raise ValueError('eztrim: the trim {} needs 2 elements'
                                     .format(trim))
                for i in trim:
                    if type(i) != int:
                        raise ValueError('eztrim: the trim {} must have 2 ints'
                                         .format(trim))
        ...
        if debug:
            return {'s':        self.s, 'e': self.e, 'cut_ts_s': self.cut_ts_s,
                    'cut_ts_e': self.cut_ts_e}

        self._cut_audio()

    def octrim(self, clip: vs.VideoNode,
               trims: Union[List[Union[Tuple[int, int, str], Tuple[int, str]]],
                            List[Union[Tuple[int, int], Tuple[int]]]],
               audio_file: str, outfile: str, chapter_file: str,
        ...
        >> trims=[(1,2,'A'),(4,'B'),(8,9,'C'),(11,'D'),(14,'E'),
                  (18,20,'F'),(24,30,'G'),(33,'H'),(36,41,'J')]
        ...
        :param outfile: can be either a filename `'out.wav'`
                        or a full path `'/path/to/out.wav'`

        :param chapter_file: can be either a filename 'chap.txt'
                             or a full path '/path/to/chap.txt'

        :param gui: whether or not to auto-open MKVToolNix GUI
                    with chapter_file (Default value = True)

        :param names: whether or not to use specified chapter names
                      (Default value = True)
        ...
        :param debug: used for testing, leave blank
                      (Default value = False)
        ...
        if names:
            for trim in trims:
                if type(trim) != tuple:
                    raise TypeError('octrim: trims must all be tuples (if '
                                    'only using start frame, enter as (int,)')
                if len(trim) > 3:
                    raise ValueError('octrim: trim {} must have at most 2 '
                                     'ints and 1 string'.format(trim))
                elif len(trim) == 3:
                    check_int = lambda a: isinstance(a, int)
                    for i in range(2):
                        if not check_int(trim[i]):
                            raise TypeError('octrim: trim {} must have 2 ints'
                                            .format(trim))
                    if not isinstance(trim[2], str):
                        raise TypeError('octrim: expected str in pos 2 in '
                                        'trim {}'.format(trim))
                elif len(trim) == 2:
                    if not isinstance(trim[0], int):
                        raise TypeError('octrim: expected int in pos 0 in '
                                        'trim {}'.format(trim))
                    if not isinstance(trim[1], str):
                        raise TypeError('octrim: expected str in pos 1 in '
                                        'trim {}'.format(trim))
                else:
                    raise ValueError('octrim: trim {} must be at least 2 '
                                     'elements long'.format(trim))

            if len(trims[-1]) != 3:
                raise ValueError('octrim: the last trim, {}, must have 3 '
                                 'elements'.format(trims[-1]))

        else:
            for trim in trims:
                if type(trim) != tuple:
                    raise TypeError('octrim: trims must all be tuples '
                                    '(if only using start frame, enter as '
                                    '(int,))')
                if len(trim) > 2:
                    raise ValueError('octrim: trim {} must have at most 2 ints'
                                     .format(trim))
                elif len(trim) == 2:
                    check_int = lambda a: isinstance(a, int)
                    for i in range(2):
                    ...
                    if not isinstance(trim[0], int):
                        raise TypeError('octrim: expected int in pos 0 in '
                                        'trim {}'.format(trim))
            if len(trims[-1]) != 2:
                raise ValueError('octrim: the last trim, {}, must have 2 ints'
                                 .format(trims[-1]))
        ...
        if names:
            for k, v in enumerate(trims):
                if type(v[1]) == str:
                    # if the 2nd value is a string, derive end time from
                    # next start
            ...
            for k, v in enumerate(trims):
                if len(v) == 1:
                    # if only start time specified, derive end time from
                    # next start
                    self.e.append(self.s[k + 1] - 1)
                else:
                    self.e.append(v[1])
        if names:
            for trim in trims:
                if len(trim) == 2:
                    # if end time not specified,
                    # 2nd value is chapter name
        ...
        if debug:
            return {'cut_s':     cut_s, 'cut_e': cut_e, 'chap_s_ts': chap_s_ts,
                    'chap_e_ts': chap_e_ts}
    ...
    def _chapter_timings(self, a: List[int] = None, b: List[int] = None) \
        ...
        while index < len(a):
            for i in range(index, len(a)):
                if a[i] != b[i - 1] + 1:
                    # shift all values starting at 'index' to remove
                    # space between index a and previous b
                    diff = a[i] - b[i - 1] - 1
                    index = i
                    break
        ...
        # last b value incremented by 1 to include the last frame in the
        # virtual ordered-chapters timeline
        b[-1] += 1
    ...
    def _combine(self, a: List[int] = None, b: List[int] = None,
                 tclip: vs.VideoNode = None) -> Tuple[List[int], List[int]]:
        """If b is consecutive with next a, remove both."""
        a = a if a else self.s
        b = b if b else self.e
        # because b is inclusive, we need next frame for the timestamp
        b = [i + 1 for i in b]
    ...
    def _cut_audio(self):
        ...
        ident = run([self.mkvmerge, '--identify', self.audio_file],
    ...
    def _f2ts(self, f: int, tclip: vs.VideoNode = None, precision: int = 9) \
            -> str:
        """Converts frame number to HH:mm:ss.nnnnnnnnn or HH:mm:ss.mmm
        timestamp based on clip's framerate."""
        if self.clip:
            n = self.clip.fps_num
            d = self.clip.fps_den
        elif tclip:
    ...
    def _negative_to_positive(self, a: List[int], b: List[int],
                              tclip: vs.VideoNode = None) \
            -> Tuple[List[int], List[int]]:
        ...
        for x, y in zip(a, b):
            if abs(x) > num_frames or abs(y) > num_frames:
                raise ValueError('{} is out of bounds'
                                 .format(max(abs(x), abs(y))))
    ...
    def _write_chapters(self, a: List[str], b: List[str],
        ...
        chapter_names = self.chapter_names if self.chapter_names \
            else list(ascii_uppercase)
